...
class VideoRAG:
    ...
    def __init__(
        ...
        if not self.openai_api_key:
            console.print(
                "[red]warning: no openai api key found. text generation will be limited.[/red]"
            )

        self.llm = None
        if self.openai_api_key:
            self.llm = ChatOpenAI(
                api_key=self.openai_api_key, model="gpt-3.5-turbo", temperature=0.7
            )
...
"""
    ...
    def process_video(
        ...
        video_path, chunked_metadatas, full_transcript = (
            self.video_processor.process_video(url, chunk_size, stride, force_reprocess)
        )
        ...
        segment_ids = self.vector_db.add_video_segments(
            chunked_metadatas, [emb.tolist() for emb in embeddings], video_id
        )

        console.print(
            Panel(
                f"[bold green]video processed successfully![/bold green]\nvideo id: {video_id}\nsegments: {len(segment_ids)}"
            )
        )
    ...
    def query(
        self, question: str, n_results: int = 3, video_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        results = self.vector_db.query_segments(
            query_embedding.tolist(), n_results=n_results, where_filter=where_filter
        )
        ...
        if self.llm:
            try:
                ...
                prompt_value = self.qa_prompt.format(
                    context=context_text, timestamps=timestamps_text, question=question
                )
    ...
    def summarize_video(
        self, video_id: Optional[str] = None, max_segments: int = 20
    ) -> str:
        """
        ...
        if results["sources"]:
            ...
            for source in results["sources"]:
                table.add_row(
                    source["timestamp"],
                    (
                        source["transcript"][:100] + "..."
                        if len(source["transcript"]) > 100
